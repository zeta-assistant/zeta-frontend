...
function getPlanFromUser(user: any): Plan {
  const raw = user?.app_metadata?.plan ?? user?.user_metadata?.plan ?? 'free';
  return raw === 'premium' ? 'premium' : 'free';
}

/** Top-left visual; pass srcOverride to force a specific image */
function ZetaPremiumMarkInline({
  plan,
  size = 104,
  srcOverride,
}: {
    ...
    <div className="inline-flex flex-col items-center">
      <div
        className={[
          ...
          isPremium ? 'shadow-[0_0_0_2px_rgba(251,191,36,0.5)]' : 'shadow-[0_0_0_2px_rgba(148,163,184,0.35)]',
        ].join(' ')}
        style={{ width: size, height: size }}
      >
        <div className={['absolute inset-0 rounded-2xl blur-xl', isPremium ? 'bg-amber-300/40 animate-pulse' : 'bg-slate-300/25'].join(' ')} />
        <Image src={src} alt="Zeta" width={size} height={size} className="relative z-10 rounded-2xl" priority />
        {isPremium && !srcOverride && (
          <>
            <svg className="absolute -top-3 -right-3 z-20 drop-shadow" width={Math.round(size * 0.42)} height={Math.round(size * 0.42)} viewBox="0 0 120 120" aria-hidden>
              <defs>
              ...
              <path d="M10 85 L30 35 L60 70 L90 25 L110 85 Z" fill="url(#goldGrad)" stroke="#B45309" strokeWidth="4" strokeLinejoin="round" />
              <rect x="18" y="85" width="84" height="16" rx="6" fill="#FCD34D" stroke="#B45309" strokeWidth="4" />
            </svg>
            <svg className="absolute -left-3 -bottom-6 z-20 animate-bounce" width={Math.round(size * 0.30)} height={Math.round(size * 0.30)} viewBox="0 0 80 80" aria-hidden>
  ...
  async function fetchAgents(uid: string) {
    ...
    try {
      ...
      if (ua?.data && ua.data.length) {
        ...
        // Fallback: derive from projects (assistant_id + type)
      ...
      // if table not found or any error, just show none
      setAgents([]);
    } finally {
    ...
    try {
      ...
      const res = await fetch('/api/profile/save', {
        ...
        body: JSON.stringify({
          ...
          profile_image_url: avatarUrl || null, // don't store default
        }),
      });
    ...
    <div className="min-h-screen bg-[#0b1226]">
      ...
      <div className="relative border-b border-white/10 bg-gradient-to-b from-indigo-950 via-indigo-900 to-indigo-800">
        <div className="absolute inset-0 pointer-events-none"
             style={{ background: 'radial-gradient(1200px 400px at 10% -10%, rgba(59,130,246,0.20), rgba(0,0,0,0)), radial-gradient(1000px 300px at 80% -20%, rgba(16,185,129,0.10), rgba(0,0,0,0))' }} />
        <div className="relative max-w-5xl mx-auto px-6 py-8 flex items-center gap-6 text-white">
          {/* Force hero avatar to the base zeta avatar */}
          <ZetaPremiumMarkInline plan={plan} srcOverride="/pantheon.png" />
          ...
          <div className="flex gap-2">
            <button onClick={() => router.push('/projects')} className="text-sm px-3 py-2 rounded-md border border-white/20 bg-white/10 hover:bg-white/15 text-white">
              ← Back to Projects
            </button>
            <button
...
What would you like Zeta to help you automate or accelerate?`}
        ...
        {/* Agents (between Profile and Premium) */}
        ...
        <div className="lg:col-span-3 bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Unlock higher capacity and power features that level up your workflow.
              </p>
            </div>
            <div className="flex items-center gap-3">
              {/* premium image included here */}
              <Image
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4 text-sm">
            <Benefit icon="⚡" title="Capacity Boost" desc="Increase project limit from 3 → 10." />
          ...
          <div className="mt-6 overflow-hidden rounded-xl border border-gray-200">
            <table className="w-full text-sm">
              ...
              <tbody>
                <Row label="Projects" free="3" pro="10" />
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <button
              onClick={handleUpgrade}
              className={['text-sm px-5 py-2 rounded-md shadow', plan === 'premium' ? 'bg-gray-200 text-gray-700 cursor-pointer' : 'bg-amber-500 text-white hover:bg-amber-600'].join(' ')}
            >
              {plan === 'premium' ? 'Manage Billing' : 'Upgrade to Premium'}
            </button>
  ...
  return (
    <div className="text-right">
      {isPremium ? (
        ...
        <div className="text-sm">
          <div className="font-semibold">Premium · <span className="text-amber-600">$30/mo</span></div>
